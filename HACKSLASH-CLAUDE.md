...
- **Hosting the server**: GitHub Pages cannot run a server. Use **Fly.io shared-cpu-1x
  (256–512 MB) with a 1 GB volume at `/data`**; SQLite lives on the volume (WAL mode,
  `synchronous=NORMAL`); nightly `VACUUM INTO /data/backup.db` plus a manual
  `fly ssh sftp get` backup. Roughly $2–4/month. There is no $0 host with a persistent
  disk, a real CPU and websockets; free tiers wipe the disk on every deploy and sleep.
  If it must be $0, use Turso/libSQL and make `db.js` async from day one. Decide before
  P3 and never change it after.
...
## Character
- **Classes (3 in v1)**: `Brawler` (melee), `Stormcaller` (elemental caster),
  `Gravecaller` (summoner/curses). Every class uses mana in v1 (the Brawler's globe is
  merely labelled "fury"); there is no separate rage system. Levels 1–30.
- **Stats**: STR, DEX, VIT, ENE; 5 stat points per level, 1 skill point per level.
  Starting STR/DEX/VIT/ENE: Brawler 25/15/20/10 · Stormcaller 10/15/15/30 ·
  Gravecaller 15/15/20/25. Life L1 / per level / per VIT and mana L1 / per level / per
  ENE: Brawler 60/+10/3 ; 15/+1/1 · Stormcaller 40/+6/2 ; 35/+3/3 · Gravecaller
  45/+7/2 ; 30/+2/2.5. Derived: armour, block, resists (fire/cold/lightning/poison),
...
- **Skill row schema**: `{id, class, tree, row, kind: melee|projectile|aoe|buff|summon|
  ...
  `sim/skills.js`. Summons: Gravecaller cap `2 + floor(rank/3)`, hard cap 6 per player;
  summons count against the zone entity budget.
- **P1 skills (Brawler, tree "Onslaught")**: Cleave (row 1, 120° arc 2.5 m, 110% wpn,
  ...
  Earthbreak ← Skullbreak. The other two Brawler trees and both other classes are
  designed to the same schema in P2/P4.
...
- **P1 — Kill and loot (offline)**: Brawler with the 6 Onslaught skills, monster AI,
...
- **P2 — Zones and persistence (offline)**: town + moor + crypt, exits, waypoints,
  ...
  Brawler complete (18 skills), localStorage save in the final document format.
...
- **P4 — Depth**: champion/boss modifiers, Stormcaller and Gravecaller trees, uniques and
